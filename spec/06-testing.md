...
- **テスト対象**: `engine/` 全モジュール + `game/input.ts` + `game/juice.ts` — すべて純 TS で、node 環境の Vitest がそのまま走る（jsdom・canvas モック不要）
- **テスト対象外（意図的）**: レンダリング・rAF ループ・Solid コンポーネント。canvas モックや `@solidjs/testing-library` 追加は ROI が低く、実機確認（各フェーズの検証基準）でカバーする
...
// 文字 → kind のマッピング、'.' = null、大文字に後置修飾で特殊
// 例: 'R>' = laserH の R、'R^' = laserV の R（実装時に読みやすい記法を確定）
```
...
- 隣接だがマッチ不成立は無効
- 隣接でマッチ成立は有効
- **レーザー同士のスワップは色マッチなしでも有効**
- レーザー単独をマッチ不成立位置へ動かすのは無効
...
- 5 個直線 → レーザーはちょうど 1 個
- **横 4 と縦 4 が交差**するセル → レーザー 1 個のみ・laserH 優先
- 生成セルはそのステップの消去集合から除外される（レーザーが生き残る）

### engine/lasers.ts — resolveLaserFires

- laserH がその行 8 セルすべてを消す / laserV が列 8 セル
- マッチで消されたレーザーが発火する
- **ビームが別のレーザーに命中 → チェーン発火**し、両方の行/列が消える
- 3 個以上のレーザーチェーンが停止する（無限ループしない）
- 発火済みレーザーのセルはちょうど 1 回だけ消去集合に入る
- レーザー同士スワップ → 交換後の位置から十字消し
